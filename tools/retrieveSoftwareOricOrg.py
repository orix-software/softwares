...
from functions.write_db_file import DecimalTo16bits
from functions.write_db_file import DecimalToBinary
from functions.write_db_file import computeVersion
from functions.write_db_file import write_db_file_v2
...
def buildMdFile(filenametap8bytesLength,dest,letter,name_software,date_software,download_platform_software,programmer_software,junk_software,id_software):
...
def BuildTape(tmpfolderRetrieveSoftware,tail,dest,letter,filenametap8bytesLength,filenametapext,destroot,destetc,name_software,date_software,download_platform_software,programmer_software,junk_software,version_bin,rombasic11,fire2_joy,fire3_joy,down_joy,right_joy,left_joy,fire1_joy,up_joy):
    ...
    buildMdFile(filenametap8bytesLength,dest,letter,name_software,date_software,download_platform_software,programmer_software,junk_software,id_software)
    buildDbFileSoftwareSingle(destetc,letter,name_software,filenametap8bytesLength,version_bin,rombasic11,fire2_joy,fire3_joy,down_joy,right_joy,left_joy,fire1_joy,up_joy)
...
def manage_download(download_file,download_platform,download_label,tmpfolderRetrieveSoftware,name_software,id_software,date_software,programmer_software,junk_software,version_bin,rombasic11,fire2_joy,fire3_joy,down_joy,right_joy,left_joy,fire1_joy,up_joy,category_software,id_download):
    ...
    if download_file!="":
        ...
        typeDsk = CheckDsk(download_file, download_platform, letter,destftdos,destetcftdos,name_software,filenametap8bytesLength,tail,tmpfolderRetrieveSoftware,date_software,programmer_software,junk_software,version_bin,rombasic11,fire2_joy,fire3_joy,down_joy,right_joy,left_joy,fire1_joy,up_joy)
        if typeDsk != 0:
...
if not os.path.exists("cache"):
    pathlib.Path("cache").mkdir(parents=True)

if not os.path.exists("cache/comments"):
    pathlib.Path("cache/comments").mkdir(parents=True)

if not os.path.exists("cache/softwares"):
    pathlib.Path("cache/softwares").mkdir(parents=True)
...
for i in range(len(datastore)):
    ...
    if start == 1:
        ...
        if download_1_file != "":
            ...
            typeDsk = CheckDsk(download_1_file,download_1_platform,letter,destftdos,destetcftdos,name_software,filenametap8bytesLength,tail,tmpfolderRetrieveSoftware,date_software,programmer_software,junk_software,version_bin,rombasic11,fire2_joy,fire3_joy,down_joy,right_joy,left_joy,fire1_joy,up_joy)
            if typeDsk != 0:
                addSoftware = filenametap8bytesLength.upper()+';'+removeFrenchChars(name_software)+'\0'
                if typeDsk == 1:
                    sedoric3_main_db_str = sedoric3_main_db_str+addSoftware
...
print("Write sedoric v (temp): ")
sedoric_number_of_db_part = 1
f = open(dest_db_sedoric + "/"+sedoric3_main_db, "wb")
f.write(DecimalToBinary(version_bin_v2))
f.write(DecimalToBinary(sedoric_number_of_db_part))
f.write(bytearray(computeVersion(),'ascii'))
f.write(DecimalTo16bits(number_of_software_sedoric))
f.write(bytearray(sedoric3_main_db_str,'ascii'))
f.write(DecimalToBinary(EOF))
f.close()

ftdos_number_of_db_part = 1
write_db_file_v2("Write ftdos binary .db ", number_of_software_ftdos, ftdos_number_of_db_part, dest_db_ftdos + "/"+ftdos_main_db, ftdos_main_db_str)
